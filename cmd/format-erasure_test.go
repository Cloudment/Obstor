...
func TestFormatErasureMigrate(t *testing.T) {
	...
	if err = os.MkdirAll(pathJoin(rootPath, obstorMetaBucket), os.FileMode(0755)); err != nil {
		t.Fatal(err)
	}

	if err = os.WriteFile(pathJoin(rootPath, obstorMetaBucket, formatConfigFile), b, os.FileMode(0644)); err != nil {
		t.Fatal(err)
	}
	...
	migratedVersion, err := formatGetBackendErasureVersion(pathJoin(rootPath, obstorMetaBucket, formatConfigFile))
	if err != nil {
		t.Fatal(err)
	}
	...
	b, err = os.ReadFile(pathJoin(rootPath, obstorMetaBucket, formatConfigFile))
	if err != nil {
		t.Fatal(err)
	}
