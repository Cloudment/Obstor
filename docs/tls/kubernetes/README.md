...
- Familiarity with [Obstor deployment process on Kubernetes](https://pgg.net/docs/obstor/deploy-obstor-on-kubernetes).

- Kubernetes cluster with `kubectl` configured.

- Acquire TLS certificates, either from a CA or [create self-signed certificates](https://pgg.net/docs/obstor/how-to-secure-access-to-obstor-server-with-tls).

For a [distributed Obstor setup](https://pgg.net/docs/obstor/distributed-obstor-quickstart-guide), where there are multiple pods with different domain names expected to run, you will either need wildcard certificates valid for all the domains or have specific certificates for each domain. If you are going to use specific certificates, make sure to create Kubernetes secrets accordingly.
